#!/bin/sh
# run_dbg-venc8.sh — RTSP boot + the low-latency chn8 H.265 TCP tap (device/venc8tap.c).
# Same as run_dbg.sh (ECM + RTSP + telnetd) but LD_PRELOADs libvenc8tap.so into arlink_fpv,
# which tees the composited chn8 H.265 (with OSD) out tcp://10.55.0.1:9000 at low latency.
# View:  tools/view-venc8.sh   (ffplay over the ECM link; forces a keyframe on connect).
#
# Original run_dbg.sh header follows:
...
if [ -e "/usr/usrdata/ar813x/start_ar813x_gnd.sh" ]; then
	...
	if [ $? -ne 0 ]; then
		...
		sleep 0.5
		LD_PRELOAD=/usrdata/libvenc8tap.so arlink_fpv -m 2 -t 0 >/dev/null &
		sleep 2
		arlink_daemon &
