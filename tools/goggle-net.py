...
Far faster than the byte-paced UART (tools/goggle.py): commands go over telnet, file pushes
...
def _negotiate(sock, data, out):
    ...
    while i < len(data):
        if data[i] == 255 and i+2 < len(data):     # IAC
            ...
            if ch == 253:   sock.sendall(bytes([255,252,opt]))   # DO  -> WONT
            elif ch == 251: sock.sendall(bytes([255,254,opt]))   # WILL-> DONT
            i += 3; continue
        out.append(data[i]); i += 1

class Net:
    def __init__(self, host, httpip):
        self.host = host; self.httpip = httpip
    def _connect(self):
        s = socket.create_connection((self.host, 23), timeout=5); s.settimeout(0.5)
        return s
    def run(self, cmd, wait=8.0):
        """Run a command, return (stdout_text, rc). Uses an expanded sentinel to skip the echo."""
        s = self._connect(); buf = bytearray()
        def pump(t):
            end = time.time() + t
            while time.time() < end:
                try:
                    ...
                    if not d: break
                    _negotiate(s, d, buf)
                    if re.search(rb"RC=\d+=DONE", buf): return True
    ...
    def push(self, local, remote, tries=3):
        ...
        tmp = tempfile.mkdtemp(); name = os.path.basename(local)
        shutil.copy(local, os.path.join(tmp, name))
        os.chdir(tmp)
        httpd = ThreadingHTTPServer((self.httpip, 0), lambda *a, **k: SimpleHTTPRequestHandler(*a, directory=tmp, **k))
        port = httpd.server_address[1]
        th = threading.Thread(target=httpd.serve_forever, daemon=True); th.start()
        try:
            url = f"http://{self.httpip}:{port}/{name}"
            for attempt in range(1, tries+1):
                out, rc = self.run(f"wget -q -O {remote} {url} && md5sum {remote}", wait=20.0)
                got = next((t for t in out.split() if len(t)==32 and all(c in '0123456789abcdef' for c in t)), "")
                print(f"  push attempt {attempt}: {'OK' if got==want else 'mismatch, retrying'}")
...
def main():
    ...
    a = ap.parse_args(); n = Net(a.host, a.httpip)
    if a.action == "run":
        out, rc = n.run(a.cmd); print(out, end=''); sys.exit(rc if rc>=0 else 1)
    elif a.action == "push":
        ok = n.push(a.local, a.remote); print("PUSH OK" if ok else "PUSH FAILED"); sys.exit(0 if ok else 1)
    elif a.action == "shell":
        n.shell()
